...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func addKeyboard(keyboardFrame:CGRect){
        ...
        // 回删的视图添加
        let curdX = 1 + 2 * btnWidth
        let curdY = 1 + 3 * btnHeight
        let numdViewFrame = CGRectMake(curdX, curdY, btnWidth - 1, btnHeight)
        let numdView = UIView(frame: numdViewFrame)
        let numdLabel = UILabel(frame: CGRectMake(0,0,numdViewFrame.width,numdViewFrame.height))
        numdLabel.text = "回删"
        numdLabel.textColor = UIColor.blueColor()
        numdLabel.textAlignment = .Center
        numdView.addSubview(numdLabel)
        numdView.backgroundColor = UIColor.whiteColor()
        numdView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(self.deleteNum)))
        keyboardView.addSubview(numdView)
    ...
    func addNumToTemp(sender:UIButton){
        ...
        if let _ = Double(numTF.text!.stringByReplacingOccurrencesOfString(",", withString: "")){
